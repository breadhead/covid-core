...
const mockCreateCard = jest
  .fn()
  .mockImplementation((title: string, description: string, listId: string) => {
    return Promise.resolve(listId === 'correct-test-board-id' ? {} : '')
  })

const mockUpdateCardList = jest
  .fn()
  .mockImplementation((cardId: string, listId: string) => {
    return Promise.resolve(listId === 'correct-test-board-id' ? {} : '')
  })

const mockAddLabelToCard = jest
  .fn()
  .mockImplementation((cardId: string, labelId: string, listId: string) => {
    return Promise.resolve(listId === 'correct-test-board-id' ? {} : '')
  })
...
describe('TrelloBoardManager', () => {
  ...
  describe('createCard', () => {
    test('api method executed with correct params', async () => {
      await trelloBoardManager.createCard(
        'test-name',
        'test-content',
        'correct-test-board-id',
      )
    ...
    test('api method executed with incorrect params', async () => {
      await expect(
        trelloBoardManager.createCard(
          'test-name',
          'test-content',
          'incorrect-test-board-id',
        ),
      ).rejects.toThrow(BoardManagerException)
    })
  })
