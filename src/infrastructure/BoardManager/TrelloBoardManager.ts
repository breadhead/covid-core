...
export default class TrelloBoardManager implements BoardManager {
  ...
  public constructor(config: Configuration) {
    this.trello = new Trello(
      config.get('TRELLO_APP_KEY').getOrElse('f3e15111456e6b40e7276b58c3ac5a33'),
      config.get('TRELLO_USER_TOKEN').getOrElse('6d7cfc89e7d946b852fc749fd996493482cb17ea895c6c086e6a586657168001'),
    )
  }

  public async createCard(name: string, content: string, listId: string): Promise<string> {
    const response = await this.trello.addCard(name, content, listId)
      .then(tapOrThrow)
    return response.id
  }

  public async moveCard(cardId: string, listId: string): Promise<void> {
    const response = await this.trello.updateCardList(cardId, listId).then(tapOrThrow)
  }
  ...
  public async setDueDate(cardId: string, due: Date): Promise<void> {
    const result = await this.trello.addDueDateToCard(cardId, due).then(tapOrThrow)
  }
  public async addMemberToCard(cardId: string, memberId: string): Promise<void> {
    const result = await this.trello.addMemberToCard(cardId, memberId).then(tapOrThrow)
  }
  ...
  public async getCard(cardId: string) {
    const card = this.trello.makeRequest('GET', '/1/card/' + cardId).then(tapOrThrow)
    return card
  }

  private async createOrGetLabel(boardId: string, labelText: string): Promise<any> {
    const labels = await this.trello.getLabelsForBoard(boardId).then(tapOrThrow)
    const label = labels.find(({name}) => name === labelText)
      || await this.trello
        .addLabelOnBoard(boardId, labelText)
        .then(tapOrThrow)
    return label
  }
}
